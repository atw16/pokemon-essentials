...
if USING_SURF_ITEM
  ...
  def pbSurf
    ...
    if !pbCheckHiddenMoveBadge(BADGE_FOR_SURF,false) && !$DEBUG
      return false
    end
    if !pbHasItem?(SURF_ITEM)
      return false
    else
...
if USING_ROCK_SMASH_ITEM
  ...
  def pbRockSmash
    if !pbCheckHiddenMoveBadge(BADGE_FOR_ROCKSMASH,false)
      pbMessage(_INTL("It's a rugged rock, but an item may be able to smash it."))
      return false
    end
    if !pbHasItem?(ROCK_SMASH_ITEM)
      pbMessage(_INTL("It's a rugged rock, but an item may be able to smash it."))
      return false
    else
...
if USING_STRENGTH_ITEM
  ...
  def pbStrength
    ...
    if !pbCheckHiddenMoveBadge(BADGE_FOR_STRENGTH,false) && !$DEBUG
